...
static struct rte_flow* generate_cm_flow(uint16_t port_id,
...
static int release_domain_resources(struct dpdk_domain_resources* res) {
    if (res) {
        ...
        if (pep_busy) {
            DPDK_WARN(FI_LOG_FABRIC,"Cannot release domain resources on %s, "
...
int create_dpdk_domain_resources(struct fi_info* info,
                                 struct dpdk_domain_resources** pres) {
    int err = FI_SUCCESS;

    if (!info->domain_attr || !info->domain_attr->name) {
        DPDK_WARN(FI_LOG_FABRIC,"Failed to create domain resources because domain name is invalid.\n");
        err = -FI_EINVAL;
        goto error_group_1;
    }
    ...
    RTE_ETH_FOREACH_DEV(port_id) {
        ...
        if (err != FI_SUCCESS) {
            DPDK_WARN(FI_LOG_FABRIC,"Cannot get information of port_id:%d, error:%s. skipping...\n",
    ...
    if (!port_found) {
        DPDK_WARN(FI_LOG_FABRIC,"Failed to create domain resources because we cannot find domain:%s.\n",
    ...
    if((err = rte_eth_dev_adjust_nb_rx_tx_desc(port_id,&nb_rxd,&nb_txd)) != 0) {
        DPDK_WARN(FI_LOG_FABRIC, "rte_eth_dev_adjust_nb_rx_tx_desc ont port:%u failed with error:%s.\n",
                  port_id,rte_strerror(rte_errno));
        goto error_group_3;
    }
    ...
    for (uint16_t q = 0; q < 2; q++) {
        if((err = rte_eth_rx_queue_setup(port_id, q, nb_rxd, socket_id, NULL, res->cm_pool)) != 0) {
            DPDK_WARN(FI_LOG_FABRIC, "rte_eth_rx_queue_setup failed on port:%u, qid:%u failed with error:%s.\n",
                      port_id, q, rte_strerror(rte_errno));
            goto error_group_3;
        }
        if((err = rte_eth_tx_queue_setup(port_id, q, nb_txd, socket_id, &txconf)) != 0) {
            DPDK_WARN(FI_LOG_FABRIC, "rte_eth_tx_queue_setup failed on port:%u, qid:%u failed with error:%s.\n",
                      port_id, q, rte_strerror(rte_errno));
            goto error_group_3;
        }
    }
    ...
    if ((res->cm_flow = generate_cm_flow(port_id,1,res->local_cm_addr.sin_addr.s_addr,res->local_cm_addr.sin_port,&flow_error)) == NULL) {
        DPDK_WARN(FI_LOG_FABRIC, "cm flow generation failed on port:%u, error:%s.\n",
                  port_id, flow_error.message?flow_error.message:"unkown");
        goto error_group_3;
    }
    // session counter
...
int get_or_create_dpdk_domain_resources(struct dpdk_fabric* fabric,
                                        struct fi_info*     info,
                                        struct dpdk_domain_resources** pres) {
    ...
    if (res) {
        ...
        if (info->src_addrlen > 0) {
            if (info->src_addrlen != sizeof(res->local_cm_addr)) {
                DPDK_WARN(FI_LOG_FABRIC,"Information does not match:info->src_addrlen(%lu)!=%lu.\n",
                          info->src_addrlen,sizeof(res->local_cm_addr));
                err = -FI_EINVAL;
            } else if (((struct sockaddr_in*)info->src_addr)->sin_family != res->local_cm_addr.sin_family ||
                       ((struct sockaddr_in*)info->src_addr)->sin_port   != res->local_cm_addr.sin_port   ||
                       ((struct sockaddr_in*)info->src_addr)->sin_addr.s_addr != res->local_cm_addr.sin_addr.s_addr) {
